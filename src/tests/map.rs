  ...
  <M::Allocator as Sealed>::Trailer: Default,
{
  ...
  for i in 0..N {
    let l1 = l.clone();
    let l2 = l.clone();
    std::thread::Builder::new().name(format!("map-concurrent-basic2-writer-{i}-1")).spawn(move || {
      let _ = l1.insert(&key(i), &new_value(i));
    }).unwrap();

    std::thread::Builder::new().name(format!("map-concurrent-basic2-writer{i}-2")).spawn(move || {
...
where
  M: Map<Comparator = dbutils::Ascend> + Clone,
  M::Comparator: Comparator,
