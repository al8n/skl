...
fn above_lower_bound_compare<C, V, T>(cmp: &C, bound: &Bound<&T>, other: &V::Ref<'_>) -> bool
where
  V: ?Sized + Type,
  T: ?Sized,
  C: TypeRefQueryComparator<V, T>,
{
  match *bound {
...
fn below_upper_bound_compare<C, V, T>(cmp: &C, bound: &Bound<&T>, other: &V::Ref<'_>) -> bool
where
  V: ?Sized + Type,
  T: ?Sized,
  C: TypeRefQueryComparator<V, T>,
