...
pub struct AllVersionsIter<'a, T, C, Q: ?Sized = &'static [u8], R = core::ops::RangeFull> {
  ...
  pub(super) last: Option<OptionEntryRef<'a, T, C>>,
...
impl<'a, Q: ?Sized, R, T, C> AllVersionsIter<'a, T, C, Q, R> {
  ...
  #[inline]
  pub const fn entry(&self) -> Option<&OptionEntryRef<'a, T, C>> {
    self.last.as_ref()
  }
}
  ...
  R: RangeBounds<Q>,
{
  ...
  pub fn seek_upper_bound(&mut self, upper: Bound<&[u8]>) -> Option<OptionEntryRef<'a, T, C>> {
    match upper {
      Bound::Included(key) => self.seek_le(key).map(|n| {
        let ent = OptionEntryRef::from_node(n, self.map);
        self.last = Some(ent);
        ent
      }),
  ...
  /// If no such element is found then `None` is returned.
  pub fn seek_lower_bound(&mut self, lower: Bound<&[u8]>) -> Option<OptionEntryRef<'a, T, C>> {
    match lower {
  ...
  fn next_in(&mut self) -> Option<OptionEntryRef<T, C>> {
    loop {
      unsafe {
  ...
  /// iterator is pointing at a valid entry, and `None` otherwise.
  fn prev(&mut self) -> Option<OptionEntryRef<T, C>> {
    loop {
      unsafe {
  ...
  fn first(&mut self) -> Option<OptionEntryRef<'a, T, C>> {
  ...
  fn last(&mut self) -> Option<OptionEntryRef<'a, T, C>> {
...
where
  ...
  R: RangeBounds<Q>,
{
  type Item = OptionEntryRef<'a, T, C>;
