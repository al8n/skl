...
pub(crate) trait Allocator: Sized + Clone + core::fmt::Debug {
  ...
  /// `munlock(ptr, len)`—Unlock memory from RAM.
  #[cfg(all(feature = "memmap", not(target_family = "wasm"), not(windows)))]
  #[cfg_attr(
    docsrs,
    doc(cfg(all(feature = "memmap", not(target_family = "wasm"), not(windows))))
  ...
  /// Rewind the underlying [`Allocator`] to the given position.
  ...
  /// Returns the magic version number.
  fn magic_version(&self) -> u16;
