...
pub trait AsList {
  ...
  fn as_list(&self) -> &SkipList<Self::Allocator, Self::Comparator>;
}

impl<T> BaseMap for T
where
  T: AsList + From<SkipList<T::Allocator, T::Comparator>>,
{
  ...
  fn magic_version(&self) -> u16 {
    self.as_list().arena.options().magic_version()
  }

  #[inline]
  fn version(&self) -> u16 {
    ArenaAllocator::magic_version(core::ops::Deref::deref(&self.as_list().arena))
  }
}

/// A trait which can be constructed from a [`Builder`](super::Builder) and some options.
pub trait BaseMap: Sized + From<SkipList<Self::Allocator, Self::Comparator>> {
