...
#[derive(Debug)]
#[doc(hidden)]
pub struct ListAllocator<H, N, A> {
  arena: A,
  max_key_size: u32,
  max_value_size: u32,
  max_height: u32,
...
impl<H, N, A: Clone> Clone for ListAllocator<H, N, A> {
  fn clone(&self) -> Self {
    Self {
...
impl<H, N, A> core::ops::Deref for ListAllocator<H, N, A> {
  type Target = A;
...
impl<H: Header, N: Node, A: rarena_allocator::Allocator + core::fmt::Debug> Sealed
  for ListAllocator<H, N, A>
{
  type Header = H;
