  ...
  RC: RefCounter,
{
  ...
  #[inline]
  pub fn iter<'a>(&'a self, version: Version) -> iterator::Iter<'a, K, LazyRef<'a, V>, A, RC, C>
  where
    K::Ref<'a>: KeyRef<'a, K>,
  {
  ...
  pub fn iter_all_versions<'a>(
    &'a self,
    version: Version,
  ) -> iterator::Iter<'a, K, Option<LazyRef<'a, V>>, A, RC, C>
  where
    K::Ref<'a>: KeyRef<'a, K>,
  {
  ...
  pub fn range<'a, Q, R>(
    ...
    version: Version,
    range: R,
  ) -> iterator::Iter<'a, K, LazyRef<'a, V>, A, RC, C, Q, R>
  where
    K::Ref<'a>: KeyRef<'a, K>,
    Q: ?Sized + Comparable<K::Ref<'a>>,
    R: RangeBounds<Q>,
  ...
  pub fn range_all_versions<'a, Q, R>(
    ...
    version: Version,
    range: R,
  ) -> iterator::Iter<'a, K, Option<LazyRef<'a, V>>, A, RC, C, Q, R>
  where
    K::Ref<'a>: KeyRef<'a, K>,
    Q: ?Sized + Comparable<K::Ref<'a>>,
    R: RangeBounds<Q>,
