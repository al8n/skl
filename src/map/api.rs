...
impl<T, C> SkipMap<T, C> {
  ...
  ///
  /// # Safety
  /// This method is not concurrency safe, invokers must ensure that no other threads are accessing the skipmap.
...
impl<T: Trailer, C: Comparator> SkipMap<T, C> {
  ...
  /// Gets or inserts a new key-value pair if it does not yet exist.
  ///
  /// - Returns `Ok(None)` if the key was successfully inserted.
  ...
  /// Gets or inserts a new key if it does not yet exist.
  ///
